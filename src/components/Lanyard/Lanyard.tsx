...
function Band({
  ...
  // Handle external shake / fling trigger
  useEffect(() => {
    if (shakeTrigger > 0 && card.current) {
      card.current.wakeUp();
      const fx = (Math.random() - 0.5) * 8;
      const fy = (Math.random() - 0.2) * 4;
      const fz = (Math.random() - 0.5) * 8;
      card.current.applyImpulse({ x: fx, y: fy, z: fz }, true);
      card.current.applyTorqueImpulse({ x: (Math.random() - 0.5) * 3, y: (Math.random() - 0.5) * 6, z: (Math.random() - 0.5) * 3 }, true);
    }
  }, [shakeTrigger]);
